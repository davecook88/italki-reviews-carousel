...
//this class takes the json for each comment and inserts the data into a generic template. This is taken directly from italki's website
//and only altered very slightly.
...
class CommentsBox{
  constructor(parentElementId){
    ...
    this.delay = 2000; //refresh rate in milliseconds
    this.timer = true; //only exists to turn off the refresh while debugging
  }
  refreshComment(i) {
      setTimeout(() => {
        let root = this.parent; //root element in the html;
        let review = new Review(reviewsJson[i]);
        if (root.firstChild) root.removeChild(root.firstChild);
        root.append(review.div);
        
        //i keeps incrementing to display all of the reviews one by one. When i is greater than the number of reviews, it reverts to 0.
